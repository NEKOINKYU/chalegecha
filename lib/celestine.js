var Celestine = (() => {
  ...
  // node_modules/celestine/dist/index.js
  var index_exports = {};
          ...
          if (apex1 === apex2) {
            if (apex1 !== opp.body1 && apex1 !== opp.body2) {
              ...
              const isDuplicate = patterns.some(
                (p) => p.type === "T-Square" && arraysEqual(p.bodies.sort(), bodies)
              );
              if (!isDuplicate) {
                patterns.push({
                  type: "T-Square",
      ...
      for (const q1 of quincunxesFromBody1) {
        ...
        for (const q2 of quincunxesFromBody2) {
          ...
          if (apex1 === apex2 && apex1 !== sextile.body1 && apex1 !== sextile.body2) {
            ...
            const isDuplicate = patterns.some(
              (p) => p.type === "Yod" && arraysEqual(p.bodies.sort(), bodies)
            );
            if (!isDuplicate) {
              patterns.push({
                type: "Yod",
    ...
    for (let i = 0; i < oppositions.length; i++) {
      for (let j = i + 1; j < oppositions.length; j++) {
        ...
        if (trine2a && trine2b) {
          ...
          if (sex1 && sex2) {
            const isDuplicate = patterns.some(
              (p) => p.type === "Mystic Rectangle" && arraysEqual(p.bodies.sort(), allBodies.sort())
            );
            if (!isDuplicate) {
              patterns.push({
                type: "Mystic Rectangle",
  ...
  function detectStellium(aspects) {
    const patterns = [];
    const conjunctions = aspects.filter(
  ...
  function eclipticToZodiac(longitude) {
    ...
    const sign = signIndex === 12 ? 0 : signIndex;
  ...
  function calculatePlanetPositions(jd, options) {
    ...
    planets.set("Sun", getSunPosition(jd));
    planets.set("Moon", getMoonPosition(jd));
    planets.set("Mercury", getMercuryPosition(jd));
    planets.set("Venus", getVenusPosition(jd));
    planets.set("Mars", getMarsPosition(jd));
    planets.set("Jupiter", getJupiterPosition(jd));
    planets.set("Saturn", getSaturnPosition(jd));
    planets.set("Uranus", getUranusPosition(jd));
    planets.set("Neptune", getNeptunePosition(jd));
    planets.set("Pluto", getPlutoPosition(jd));
  ...
  function getBodyName(body) {
    const nameMap = {
      ...
      [
        "Lilith"
        /* Lilith */
      ]: BODY_NAMES.meanLilith,
      [
        "TrueLilith"
        /* TrueLilith */
      ]: BODY_NAMES.trueLilith
    };
  ...
  function calculateChart(birth, options) {
    ...
    const sunPos = positions.planets.get(
      "Sun"
      /* Sun */
    );
    ...
    if (positions.chiron) {
      planets.push(buildChartPlanet("Chiron", positions.chiron, houseResult.houses));
    }
    if (positions.asteroids) {
      if (positions.asteroids.ceres) {
        planets.push(
          buildChartPlanet("Ceres", positions.asteroids.ceres, houseResult.houses)
        );
      }
      if (positions.asteroids.pallas) {
        planets.push(
          buildChartPlanet("Pallas", positions.asteroids.pallas, houseResult.houses)
        );
      }
      if (positions.asteroids.juno) {
        planets.push(
          buildChartPlanet("Juno", positions.asteroids.juno, houseResult.houses)
        );
      }
      if (positions.asteroids.vesta) {
        planets.push(
          buildChartPlanet("Vesta", positions.asteroids.vesta, houseResult.houses)
        );
      }
    }
    const nodes = [];
    ...
    if (mergedOptions.includeLots && sunPos) {
      const moonPos = positions.planets.get(
        "Moon"
        /* Moon */
      );
      if (moonPos) {
  ...
  function calculatePlanets(birth, options) {
    ...
    if (mergedOptions.includeChiron && positions.chiron) {
      planets.push(buildChartPlanet("Chiron", positions.chiron, placeholderHouses));
    }
    if (mergedOptions.includeAsteroids && positions.asteroids) {
      if (positions.asteroids.ceres) {
        planets.push(
          buildChartPlanet("Ceres", positions.asteroids.ceres, placeholderHouses)
        );
      }
      if (positions.asteroids.pallas) {
        planets.push(
          buildChartPlanet("Pallas", positions.asteroids.pallas, placeholderHouses)
        );
      }
      if (positions.asteroids.juno) {
        planets.push(
          buildChartPlanet("Juno", positions.asteroids.juno, placeholderHouses)
        );
      }
      if (positions.asteroids.vesta) {
        planets.push(
          buildChartPlanet("Vesta", positions.asteroids.vesta, placeholderHouses)
        );
      }
    }
    return planets;
  ...
  __export2(progressions_exports, {
  ...
  var TYPICAL_RETROGRADE_DURATIONS = {
    ...
    [
      "Mercury"
      /* Mercury */
    ]: { avgDays: 21, minDays: 19, maxDays: 24 },
    [
      "Venus"
      /* Venus */
    ]: { avgDays: 42, minDays: 40, maxDays: 43 },
    [
      "Mars"
      /* Mars */
    ]: { avgDays: 72, minDays: 58, maxDays: 81 },
    [
      "Jupiter"
      /* Jupiter */
    ]: { avgDays: 121, minDays: 118, maxDays: 124 },
    [
      "Saturn"
      /* Saturn */
    ]: { avgDays: 140, minDays: 137, maxDays: 142 },
    [
      "Uranus"
      /* Uranus */
    ]: { avgDays: 150, minDays: 148, maxDays: 152 },
    [
      "Neptune"
      /* Neptune */
    ]: { avgDays: 158, minDays: 156, maxDays: 160 },
    [
      "Pluto"
      /* Pluto */
    ]: { avgDays: 160, minDays: 156, maxDays: 164 },
    [
      "Chiron"
      /* Chiron */
    ]: { avgDays: 140, minDays: 135, maxDays: 145 },
    ...
    [
      "Ceres"
      /* Ceres */
    ]: { avgDays: 90, minDays: 80, maxDays: 100 },
    [
      "Pallas"
      /* Pallas */
    ]: { avgDays: 85, minDays: 75, maxDays: 95 },
    [
      "Juno"
      /* Juno */
    ]: { avgDays: 80, minDays: 70, maxDays: 90 },
    [
      "Vesta"
      /* Vesta */
    ]: { avgDays: 75, minDays: 65, maxDays: 85 }
  };
